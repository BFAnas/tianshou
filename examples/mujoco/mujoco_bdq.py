...
import numpy as np
from mujoco_env import make_discrete_mujoco_env
import torch
...
def get_args():
    ...
    parser.add_argument("--action-per-branch", type=int, default=10)
    # training hyperparameters
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--eps-test", type=float, default=0.01)
    parser.add_argument("--eps-train", type=float, default=0.99)
    parser.add_argument("--eps-decay", type=float, default=75e-4)
...
def test_bdq(args=get_args()):
    env, train_envs, test_envs = make_discrete_mujoco_env(
        args.task, args.seed, args.training_num, args.test_num, obs_norm=False, action_per_branch=args.action_per_branch
    )
    ...
    def train_fn(epoch, env_step):  # exp decay
        ...
        writer.add_scalar("update/epsilon", eps, env_step)
        policy.set_eps(eps)
    ...
    # save the buffer
    train_collector.buffer.save(os.path.join(log_path, "buffer.pth"))
