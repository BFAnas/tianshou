...
class RDPGPolicy(DDPGPolicy):
    ...
    def _target_z(self, batch: RolloutBatchProtocol) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        act_next_pi = obs_next_result.act
        taus_hat, presum_taus = self._get_taus(
            batch_size, self._n_taus, batch.obs.dtype
        )
        next_z = self.critic_old(batch.obs_next, act_next_pi, taus_hat)  # (bsz, n_taus)
        target_z = (
    ...
    def _critic_loss(
        ...
        re_loss = ((1 - in_data) * torch.max(current_z_pi - current_z, torch.zeros_like(current_z_pi))).mean()
