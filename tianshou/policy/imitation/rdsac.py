...
class RDSACPolicy(SACPolicy):
    ...
    def __init__(
        ...
        regularization_weight: float = 5.0,
        q_marge: float = 5.0,
        n_taus: int = 16,
    ...
    def train(self, mode: bool = True) -> "RDSACPolicy":
        self.training = mode
    ...
    def _critic_loss(
        ...
        current_z_pi = critic(batch.obs, act_pi, taus_hat_j)
        ...
        loss = qr_loss + re_loss

        return loss, current_z, in_data, qr_loss, re_loss
    ...
    def learn(self, batch: RolloutBatchProtocol, **kwargs: Any) -> Dict[str, float]:
        ...
        critic1_loss, current_z1, in_data1, qr_loss1, re_loss1 = self._critic_loss(batch, self.critic1, taus_hat_j)
        critic2_loss, current_z2, in_data2, qr_loss2, re_loss2 = self._critic_loss(batch, self.critic2, taus_hat_j)
        ...
        result = {
            ...
            "loss/quantile1": qr_loss1.item(),
            "loss/quantile2": qr_loss2.item(),
            "loss/regularization1": re_loss1.item(),
            "loss/regularization2": re_loss2.item(),
            "q_dataset1": current_z1.mean().item(),
            "q_dataset2": current_z2.mean().item(),
            "in_data1": in_data1.mean().item(),
            "in_data2": in_data2.mean().item(),
        }
        if self._is_auto_alpha:
