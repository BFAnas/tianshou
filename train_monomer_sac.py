...
from tianshou.env.venvs import ShmemVectorEnv
import torch
...
def get_args():
    ...
    parser.add_argument("--norm-layer", default=True, action="store_true")
...
def test_sac(args=get_args()):
    ...
    train_envs = VectorEnvNormObs(ShmemVectorEnv([lambda: DoMPC_Poly_env(render=args.render, randomize=args.randomize, hard_constraint=args.hard_constraint, penalties=args.penalties) for _ in range(args.training_num)]))
    test_envs = VectorEnvNormObs(ShmemVectorEnv([lambda: DoMPC_Poly_env(render=args.render, randomize=args.randomize, hard_constraint=args.hard_constraint, penalties=args.penalties) for _ in range(args.test_num)]))
